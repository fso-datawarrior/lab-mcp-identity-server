...
| AD-19 | The documented environment kill-switch disables live access on the desktop path. | The desktop launcher hard-codes the real client mode into the process environment, overriding the value the runbook tells the operator to change. | Edit the launcher line, or make the launcher honor the setting; recommendation recorded. | Named runbook gap 2026-07-15; fails safe (defaults to real) |
