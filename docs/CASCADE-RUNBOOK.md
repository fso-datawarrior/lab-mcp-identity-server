...
Live M4-c cascade evidence: see Claude project doc `Lab3-M4c-Cascade-Timeline-Record-2026-07-13.md`.
...
`--scim-id` overrides Lab 1 deprovision matching when multiple PATCH lines exist, when the joiner-leaver run has no PUT (POST create only), or when Lab 1 emits a lossy audit line (AD-17).

### AD-17: lossy Lab 1 deprovision audit

Lab 1 sometimes logs deprovision PATCH operations as only `{"op":"replace"}` with no `path` or `value` on the operation object. The strict correlator requires `active:false` and would false-negative on real runs.

When `--scim-id` is supplied and exactly one PATCH on `/scim/v2/Users/<scim-id>` contains a bare `op:replace`, the correlator accepts it with `matchMethod: scim-id-replace-unconfirmed` and prints a stderr caveat that `active:false` could not be confirmed from the lossy audit.

Without `--scim-id`, bare `replace` PATCH lines still fail closed (they could be a re-activation).

Root-cause fix belongs in [lab-okta-scim-server](https://github.com/fso-datawarrior/lab-okta-scim-server): log full `op`, `path`, and `value` on PATCH operations.
...
| Lossy Lab 1 PATCH audit (AD-17) | Pass `--scim-id`; expect `scim-id-replace-unconfirmed` and stderr caveat |
